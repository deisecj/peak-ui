...
const ModalCulturaRatings = ({ onCompleteStep, onBackStep }) => {
  const [characteristicsWorkplace, setCharacteristicsWorkplace] = useState([]);
  const [ratingsWorkplace, setRatingsWorkplace] = useState(new Map());
  ...
  const getCharacteristicsData = async () => {
    ...
    setCharacteristicsWorkplace(characteristicWorkplaceData);
  ...
  const handleComment = (event, characteristicID) => {
    ...
    console.log("state value", ratingsWorkplace)
  }
  ...
  return (
    <div>
      ...
      {characteristicsWorkplace.map(workplace => (
        <div key={`mw-${workplace.characteristicID}`} className="border-t border-neutral-300 mt-14">
        <div className="flex">
          <div>
            <p className="pt-6 mb-2.5 text-lg font-semibold leading-6 text-neutral-900">{workplace.name}</p>
          </div>
          <div className="mt-5">
        ...
        <div className="h-14 rounded-md shadown-xl bg-white">
          <RatingReview rateSelected={(ratingSaved) => handleRatingStar(ratingSaved, workplace.characteristicID)}/>
        </div>
        ...
        <div className="mt-3">
          <textarea
            onChange={(event) => handleComment(event, workplace.characteristicID)}
            rows={4}
            name="comment"
            id="comment"
