...
const CharacteristicRating = ({ characteristic, ratings }) => {
  return (
    <div className="workplace-experience-section mt-6 w-full">
      ...
      <div className="rating-section grid grid-cols-2 justify-between mt-5 xl:mt-10">
        {ratings.map((item, index) => (<RatingItem className={(index + 1) % 2 === 0 ? "ml-6 xl:ml-10" : "mr-6 xl:mr-10"} key={`cultural-${item.name}`} itemName={item.name} />))}
      </div>
    </div>
  );
}
